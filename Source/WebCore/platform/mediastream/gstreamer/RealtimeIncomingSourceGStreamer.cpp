...
void RealtimeIncomingSourceGStreamer::lockValve() const
{
    if (m_valve)
        g_object_set(m_valve.get(), "drop", true, nullptr);
}

void RealtimeIncomingSourceGStreamer::releaseValve() const
{
    if (m_valve)
        g_object_set(m_valve.get(), "drop", false, nullptr);
}

GstElement* RealtimeIncomingSourceGStreamer::registerClient()
{
    GST_DEBUG("Registering new client");
    auto queue = gst_element_factory_make("queue", nullptr);
    auto proxy = createProxy();

    gst_bin_add_many(GST_BIN_CAST(m_bin.get()), queue, proxy.sink, nullptr);
    gst_element_link_many(m_tee.get(), queue, proxy.sink, nullptr);
    gst_element_sync_state_with_parent(queue);
    gst_element_sync_state_with_parent(proxy.sink);

    WTF::String dotFileName = makeString(GST_OBJECT_NAME(m_bin.get()), ".incoming");
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN(m_bin.get()), GST_DEBUG_GRAPH_SHOW_ALL, dotFileName.utf8().data());
