...
static inline Optional<RTCIceProtocol> toRTCIceProtocol(const String& protocol)
{
...
static inline Optional<RTCIceTcpCandidateType> toRTCIceTcpCandidateType(const String& type)
{
...
static inline Optional<RTCIceCandidateType> toRTCIceCandidateType(const String& type)
{
    if (!type || type.isEmpty())
        return {};
    if (type == "host")
...
Optional<Ref<RTCCertificate>> generateCertificate(Ref<SecurityOrigin>&& origin, const PeerConnectionBackend::CertificateInformation& info)
{
    ...
    auto commonName = base64Encode(buffer);
    ...
    uint64_t expires = info.expires.valueOr(2592000);
